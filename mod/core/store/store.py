...
class Store:
    ...
    def get(self, path, default=None, max_age=None, update=False, key=None, verbose=False):
        """
    ...
    def path2age(self, path=None):
        """
        returns the age of the item in seconds
        """
        path2time = self.path2time(path=path)
        path2age = {}
        for p,t in path2time.items():
            path2age[p] = time.time() - t
        return path2age
    def path2time(self, path=None):
        """
