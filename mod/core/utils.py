...
def random_word( *args, n=1, seperator='_', **kwargs):
    import mod as m
    random_words = m.mod('key').generate_mnemonic(*args, **kwargs).split(' ')[0]
...
def version( lib:str=None):
    import mod as m
    lib = lib or m.repo_name
    lines = [l for l in m.cmd(f'pip3 list', verbose=False).split('\n') if l.startswith(lib)]
    if len(lines)>0:
...
def pip_install(lib:str= None,
    ...
    if m.path_exists(lib):
        ...
        if upgrade:
            cmd += ' --upgrade'
    return m.cmd(cmd, verbose=verbose)
...
def pip_list(lib=None):
    import mod as m
    lib = lib or m.repo_name
    pip_list =  m.cmd(f'pip list', verbose=False, bash=True).split('\n')
    if lib != None:
...
def module2size(search=None, depth=10000, **kwargs):
    import mod as m
    module2size = {}
    module2code = m.module2code(search=search, depth=depth, **kwargs)
...
def get_available_ports(port_range: List[int] = None , ip:str =None) -> int:
    import mod as m
    port_range = m.resolve_port_range(port_range)
    ip = ip if ip else '0.0.0.0'
    available_ports = []
    # return only when the port is available
    for port in range(*port_range): 
        if not m.port_used(port=port, ip=ip):
...
def ip(max_age=None, update:bool = False, **kwargs) -> str:
    ...
    try:
        import mod as m
        path = m.get_path('ip')
        ip = m.get(path, None, max_age=max_age, update=update)
        if ip == None:
            ip = external_ip()
            m.put(path, ip)
    except Exception as e:
...
def external_ip( default_ip='0.0.0.0') -> str:
    ...
    ip = None
    try:
        ip = m.cmd('curl -s ifconfig.me')
        assert isinstance(m.ip_to_int(ip), int)
    except Exception as e:
        print(e)
...
def public_ports(timeout=1.0):
    ...
    for port in free_ports():
        m.print(f'Checking port {port}')
        futures += [m.submit(is_port_public, {'port':port}, timeout=timeout)]
...
def random_port(*args, **kwargs):
    import mod as m
    return m.choice(m.free_ports(*args, **kwargs))
...
def free_port(ports = None,
    ...
    for port in ports: 
        if port in avoid_ports:
            continue
        if port_available(port=port, ip=ip):
            return port
...
def check_used_ports(start_port = 8501, end_port = 8600, timeout=5):
    ...
    used_ports = {}
    for port in range(*port_range):
        used_ports[port] = m.port_used(port)
    return used_ports
...
def is_url( address:str) -> bool:
    ...
    conds.append(m.is_int(address.split(':')[-1]))
    return all(conds)
...
def cuda_available() -> bool:
    import mod as m
    return m.util('hardware.cuda_available')
...
def hidden_files(path:str='./')-> List[str]:
    import mod as m
    path = os.path.abspath(path)
    files = [f[len(path)+1:] for f in  m.glob(path)]
...
def sumtext( text, split_size=100000) -> List[str]:
    ...
    for idx, chunk in enumerate(chunks):
        future = m.submit(m.cond, [chunk])
        future2idx[future] = idx
    for f in m.as_completed(future2idx):
...
def getsourcelines( module = None, search=None, *args, **kwargs) -> Union[str, Dict[str, str]]:
    ...
    if module != None:
        if isinstance(module, str) and '/' in module:
            ...
            module = getattr(m.mod(module), fn)
        else:
