...
fmt: ## Format Rust, Python and TypeScript projects if present
	...
	@if [ -f telemetry-module/Cargo.toml ]; then \
		echo "[fmt] telemetry-module"; \
		(cd telemetry-module && cargo fmt --all) || true; \
	fi
...
lint: ## Lint Rust, Python and TypeScript projects if present
	...
	@if [ -f telemetry-module/Cargo.toml ]; then \
		echo "[lint] telemetry-module"; \
		(cd telemetry-module && cargo clippy --all-targets --all-features -D warnings) || true; \
	fi
	@if command -v ruff >/dev/null 2>&1; then \
...
build-rust: ## Build Rust workspaces (if present)
	...
	@if [ -f telemetry-module/Cargo.toml ]; then echo "[build] telemetry-module"; (cd telemetry-module && cargo build --release); fi
...
build-telemetry: ## Build Rust telemetry module
	@if [ -f telemetry-module/Cargo.toml ]; then echo "[build] telemetry-module"; (cd telemetry-module && cargo build --release); else echo "[warn] telemetry-module not found"; fi
...
ci: ## CI entrypoint
	...
	@$(MAKE) check
	@$(MAKE) build-rust build-telemetry build-bridge build-ipfs
